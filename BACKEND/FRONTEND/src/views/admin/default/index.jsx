...
const Dashboard = () => {
  ...
  // You can add more state for other tables as needed
  ...
  useEffect(() => {
    fetch("http://localhost:4000/api/products")
      ...
      .then((data) => {
        ...
        // Example: [{ name: [productName, false], quantity: 0, date: '', progress: 0 }]
        const formatted = data.map((item) => ({
          name: [item.name || item.product_name || "", false],
          quantity: item.quantity || 0,
          date: item.date || "",
          progress: item.progress || 0,
        }));
        setTableDataCheck(formatted);
      })
      .catch(() => setTableDataCheck([]));
  }, []);
  ...
  return (
    <div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-2">
        ...
        <ComplexTable columnsData={columnsDataComplex} tableData={[]} />
        {/* You can wire up ComplexTable similarly if you add a backend endpoint */}
